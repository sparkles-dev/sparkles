...
import { Component, ContentChildren, QueryList, TemplateRef, AfterContentInit, ViewChild } from '@angular/core';
...
describe('<ng-template spTemplate>', () => {
  ...
  it(`<ng-template spTemplate>`, () => {
    const foo = createTestComponent(TestingComponent, `
      <sp-parent>
  ...
  it(`should throw when no <ng-template>`, () => {
    expect(() => {
      const foo = createTestComponent(TestingComponent, `<h1 spTemplate="body"></h1>`);
      foo.detectChanges();
    }).toThrow();
  });
});
